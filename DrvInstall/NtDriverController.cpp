...
CNtDriverController::CNtDriverController(CString sName,BOOL bStart,BOOL bNeedDelStopDriver)
{
	...
	m_hDriver = NULL;
	Open();	// 赋值 m_hSCM
	// 检测 *.sys 文件并复制到%system32%\drivers目录下
	Init(sName);

	//--------------------------------------------
	// 说明:  将下面的安装部分注掉，按照指定的方式安装，不要自己乱装。
	// 时间: 2011/08/12
	// Add by SongQX
	//--------------------------------------------
/*
...
#ifdef _AMD64_
		sDefaultPath.Format("%s\\x64\\%s.sys", GetDefaultPath(),m_szName);
#else
		sDefaultPath.Format("%s\\x86\\%s.sys", GetDefaultPath(),m_szName);
#endif
		if(!IsSameFile(sPath,sDefaultPath))
		{
...
BOOL CNtDriverController::StartDriver(CString sName /* = "" */)
{
	...
	if (Open())
	{
		...
		{
			WriteLogEx("!!CNtDriverController::Start,2,OpenService [%s] fail,err=%u", sName, GetLastError());
		}
	} // if
	return bResult;
}

DWORD CNtDriverController::SetStartSvcType(CString strSvcName,DWORD dwstarttype)
{
...
BOOL CNtDriverController::IsSameFile(CString sFile1, CString sFile2)
{
	BOOL bRet = FALSE;
	CFileStatus st1,st2;
...
CString CNtDriverController::GetDefaultPath()
{
	TCHAR path[MAX_PATH];
	CString sDefaultPath;
...
BOOL CNtDriverController::Stop(CString sName)
{	
	BOOL bResult=TRUE;

	if(Open())
	{
		SC_HANDLE hSvc = ::OpenService(m_hSCM,sName,SERVICE_ALL_ACCESS);
		if(hSvc)
		{
			SERVICE_STATUS serviceStatus = { 0 };
			// Notifies a service that it should stop. 
			bResult = ::ControlService(hSvc, SERVICE_CONTROL_STOP, &serviceStatus);
			if (bResult)
			{		
				bResult = WaitForState(SERVICE_STOPPED, &serviceStatus);	
			}
			...
			{
				//WriteLogEx("!!CNtDriverController::Stop,1,ControlService [%s] fail,err=%u", sName, GetLastError());
				bResult = (::GetLastError() == ERROR_SERVICE_NOT_ACTIVE);
			}
			::CloseServiceHandle(hSvc);
		}
		else
		{
			WriteLogEx("!!CNtDriverController::Stop,2,OpenService [%s] fail,err=%u", sName, GetLastError());
		}
	}
