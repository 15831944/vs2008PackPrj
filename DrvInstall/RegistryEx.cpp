...
CRegistryEx::CRegistryEx(int nMode, BOOL bReadOnly)
{
...
BOOL CRegistryEx::DeleteTree (LPCTSTR pszTree)
{
#if 1 	//add by zfq,2015-09-04,RegDeleteTree函数导致在WinXP及Win2k3下LoadLibrary本DLL失败
	BOOL BRet = RegDelnode(m_hKey, (LPTSTR)pszTree);
	if(!BRet)
	{
		WriteLogEx("!!CRegistryEx::DeleteTree,1,RegDelnode fail,pszTree=%s", pszTree);
	}
	else
	{
		WriteLogEx("CRegistryEx::DeleteTree,2,RegDelnode OK,pszTree=%s", pszTree);
	}
	return BRet;
#else
	if (!CCommonFun::IsWinVistaOrLater())
	{
		WriteLogEx("!!CRegistryEx::DeleteTree");
		return TRUE;
	}
	if (!m_hKey)
		return FALSE;

	LONG lReturn = RegDeleteTree(m_hKey, pszTree);
