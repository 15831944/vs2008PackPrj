								...
								const char *password/*=NULL*/)
{
	...
	try
	{
		...
		{
			WriteLogEx("!!CSmtp::ConnectRemoteServer,1,socket fail,err=%d", GetLastError());
			throw ECSmtp(ECSmtp::WSA_INVALID_SOCKET);
		}
		...
		if((sockAddr.sin_addr.s_addr = inet_addr(szServer)) == INADDR_NONE)
		{
			...
			else
			{
				...
				closesocket(hSocket);
#endif
				WriteLogEx("!!CSmtp::ConnectRemoteServer,2,gethostbyname fail,err=%d", GetLastError());
...
#endif
		{
			...
			closesocket(hSocket);
#endif
			WriteLogEx("!!CSmtp::ConnectRemoteServer,3,ioctlsocket fail,err=%d", GetLastError());
		...
		if(connect(hSocket,(LPSOCKADDR)&sockAddr,sizeof(sockAddr)) == SOCKET_ERROR)
		{
...
#endif
			{
				...
				closesocket(hSocket);
#endif
				WriteLogEx("!!CSmtp::ConnectRemoteServer,4,connect fail,err=%d,szServer=%s,nPort_=%d", GetLastError(), szServer, nPort_);
				throw ECSmtp(ECSmtp::WSA_CONNECT);
			}
		}
#if 0 //delete by zfq,2012.12.26
		else
			return true;
#endif  //delete by zfq,2012.12.26
		...
		while(true)
		{
			...
			if((res = select(hSocket+1,NULL,&fdwrite,&fdexcept,&timeout)) == SOCKET_ERROR)
			{
				...
				closesocket(hSocket);
#endif
				WriteLogEx("!!CSmtp::ConnectRemoteServer,13,select fail,err=%d", GetLastError());
			...
			if(!res)
			{
				...
				closesocket(hSocket);
#endif
				WriteLogEx("!!CSmtp::ConnectRemoteServer,14,select fail,err=%d", GetLastError());
		...
		if(authenticate && IsKeywordSupported(RecvBuf, "AUTH") == true)
		{
			...
			if(!m_sLogin.size())
			{
				WriteLogEx("!!CSmtp::ConnectRemoteServer,14,m_sLogin.size=%d", m_sLogin.size());
			...
			if(!m_sPassword.size())
			{
				WriteLogEx("!!CSmtp::ConnectRemoteServer,15,m_sPassword.size=%d", m_sPassword.size());
			...
			else if(IsKeywordSupported(RecvBuf, "CRAM-MD5") == true)
			{
				...
				{
					WriteLogEx("!!CSmtp::ConnectRemoteServer,20,ustrChallenge=%p,ustrPassword=%p", ustrChallenge, ustrPassword);
			...
			else if(IsKeywordSupported(RecvBuf, "DIGEST-MD5") == true)
			{
				...
				if(find<0)
				{
					WriteLogEx("!!CSmtp::ConnectRemoteServer,30,find=%d", find);
				...
				if(find<0)
				{
					WriteLogEx("!!CSmtp::ConnectRemoteServer,31,find=%d", find);
					throw ECSmtp(ECSmtp::BAD_DIGEST_RESPONSE);
				}
				...
				if(find>=0){
					...
					if(find<0)
					{
						WriteLogEx("!!CSmtp::ConnectRemoteServer,33,find=%d", find);
						throw ECSmtp(ECSmtp::BAD_DIGEST_RESPONSE);
					}
				...
				{
					WriteLogEx("!!CSmtp::ConnectRemoteServer,40,getpeername fail,err=%d", GetLastError());
				...
				{
					WriteLogEx("!!CSmtp::ConnectRemoteServer,41,ustrRealm=%p,ustrUsername=%p,ustrPassword=%p,ustrNonce=%p,ustrCNonce=%p,ustrUri=%p,ustrNc=%p,ustrQop=%p"
						, ustrRealm, ustrUsername, ustrPassword, ustrNonce, ustrCNonce, ustrUri, ustrNc, ustrQop);
					throw ECSmtp(ECSmtp::BAD_LOGIN_PASSWORD);
				}
		...
		{
			WriteLogEx("!!CSmtp::ConnectRemoteServer,100,authenticate=%d,RecvBuf=%s", authenticate, RecvBuf);
		}
	}
	catch(const ECSmtp&)
	{
		WriteLogEx("!!CSmtp::ConnectRemoteServer,101,catch sth,RecvBuf=%s", authenticate, RecvBuf);
